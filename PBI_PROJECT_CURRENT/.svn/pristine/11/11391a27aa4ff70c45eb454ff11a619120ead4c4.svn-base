...
/****** Object:  View [DPO].[AGENCY_SFC]    Script Date: 3/9/2022 11:18:27 AM ******/
SET ANSI_NULLS ON
GO
...
ON A.Agent_Number = B.[Issuing Agent]
...
, Last_Servicing_Policy1 AS (SELECT [AD_Code]
      ,[AD_Office]
      ,[Agent_Number]
      ,[ExplodedDate]
      ,[GRADE]
 ...
 ,Last_Servicing_Policy2 AS (
  SELECT -- [RN]
      ...
      --,[Agent_Name]
	  ,[PAID_TO_DATE]
	  ,IIF([Agent_Number] <> [Issuing Agent] AND [FROM_DATE] = [Issued Date], [Issuing Agent],[Agent_Number]) AS Agent_Number
	  ,[FROM_DATE]
      ,[TO_DATE]
      --,EOMONTH([FROM_DATE])
      --,EOMONTH([TO_DATE])
 ...
 ,Last_Servicing_Policy_End AS (
  ...
  FROM Last_Servicing_Policy1 AS A LEFT JOIN Last_Servicing_Policy2 AS B
  ON A.Agent_Number = B.[Agent_Number]
  AND EOMONTH(A.ExplodedDate) > = B.FROM_DATE 
  AND EOMONTH(A.ExplodedDate) < IIF(B.TO_DATE IS NULL, IIF(B.[POLICY_STATUS] <> 'IF' AND B.[POLICY_STATUS] <> 'FL',DATEADD(MONTH,2,B.[PAID_TO_DATE]),'2099-12-31')
...
SELECT
	...
	,IIF(EOMONTH(A.ExplodedDate) > = EOMONTH(B.Date_Appointed) AND EOMONTH(A.ExplodedDate) < IIF(B.Terminated_date IS NULL, '2099-12-31',B.Terminated_date), 1,0) AS ENFORCE_MONTH
	
	--,P.Policy_Number
	,IIF(A.Grade = 'IC'
	AND A.Agent_Number NOT LIKE '6999%'
	AND (C.Last_Servicing_Policy IS NOT NULL)
	AND C.AD_Office NOT IN ('SEP','DR1','ACB')
	AND DATEDIFF(MONTH, IIF(A.Last_Issued_Date IS NULL OR B.Date_Appointed > A.Last_Issued_Date, B.Date_Appointed, A.Last_Issued_Date), A.ExplodedDate) > 9
	AND A.ExplodedDate >= '2021-02-01' 
	AND ((IIF(B.Terminated_date IS NULL, '2099-12-31',B.Terminated_date)) > EOMONTH(A.ExplodedDate))
	, 'S','') AS SFC
	,A.Grade
	,B.Date_Appointed
	,B.Terminated_date
	...
	,IIF(DATEDIFF(MONTH, IIF(A.Last_Issued_Date IS NULL OR B.Date_Appointed > A.Last_Issued_Date, B.Date_Appointed, A.Last_Issued_Date), A.ExplodedDate)<0,0,
	      DATEDIFF(MONTH, IIF(A.Last_Issued_Date IS NULL OR B.Date_Appointed > A.Last_Issued_Date, B.Date_Appointed, A.Last_Issued_Date), A.ExplodedDate)) AS MONTH_INACTIVE	
...
LEFT JOIN Last_Servicing_Policy_End AS C
ON A.ID_AGENT = C.ID_AGENT

LEFT JOIN PolicyStatus AS D
ON A.Last_Issued_Policy = D.POLICY_NUMBER

LEFT JOIN PolicyStatus AS E
ON C.Last_Servicing_Policy = E.POLICY_NUMBER
