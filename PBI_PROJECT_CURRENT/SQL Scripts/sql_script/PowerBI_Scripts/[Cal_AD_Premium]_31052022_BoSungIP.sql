...
/****** Object:  View [DPO].[Cal_AD_Premium]    Script Date: 30/05/2022 13:05:07 ******/
SET ANSI_NULLS ON
GO
...
SELECT  A.[Policy No]
		...
		(SELECT TAPSU_LIST.[AGENT CODE] FROM TAPSU_LIST),NULL,A.[Current Agent Effect From]) AS Transfer_Date

  FROM Transfer_Policy_1 AS A
  LEFT JOIN Main_POLICY_INFO AS B
  ON A.[Policy No] = B.POLICY_NUMBER

  LEFT JOIN [Main_AGENT_INFO_DA] AS C
  ON A.[Current Agent] = C.Agent_Number
  )
,Transfer_Policy  AS (
  SELECT A.[Policy No]
...
select [Agent Code]
      ...
      ,[Component_Code]
      ,[After_Discount_Premium]
from DP_DA_Daily_DC_PO_WD_NT) as A
) 

,Temp2 as (select
concat(Policy_Number,Component_Code) as ID_Policy_Component,
SUM(After_Discount_Premium) AS After_Discount_Premium_IP
from Temp3
GROUP BY Policy_Number,Component_Code)

, B as (SELECT
		CONCAT([Policy No], [Product Code]) as ID_Policy_Component
      ,[Policy No]
      ,[Product Code]
...
, C as (Select B.ID_Policy_Component,
B.[Policy No],
B.Min_Collected_Date,
B.[Product Code],
Temp2.After_Discount_Premium_IP
from B join Temp2 
on B.ID_Policy_Component = Temp2.ID_Policy_Component)
--- end add IP
,Temp1 AS (
   SELECT VP.[Area Code], VP.[Policy No],VP.[Product Code], VP.[Collected Date], VP.[Issued Date], VP.[Proposal Receive Date], VP.[Policy Status], VP.[Issuing Agent], VP.[Servicing Agent],VP.[SFC], VP.FYP, VP.RYP, VP.Topup, VP.AFYP, VP.[POLICY ACKNOWLED],VP.[Freelook]
		...
		, CASE
			WHEN A.ID_Card = P.PO_IDNUMBER THEN 'SELF_OWNER'
			WHEN P.PO_IDNUMBER IN (SELECT ID_Card FROM [PowerBI].[DPO].[Main_AGENT_INFO_DA] )
			...
			ELSE 'CUSTOMER'
		 END AS Cross_Sales
		,IIF (VP.[Collected Date] < A.Date_Appointed OR A.License_No = 'TAPSU', 1,0) AS TAPSU_SALES
   ...
   LEFT OUTER JOIN Transfer_Policy AS T
   ON VP.[Policy No] = T.[Policy No]
)
SELECT
	Temp1.*,
	C.After_Discount_Premium_IP
FROM Temp1
left join C on Temp1.[Policy No] = C.[Policy No] and
  Temp1.[Product Code] = C.[Product Code] and
  Temp1.[Collected Date] = C.Min_Collected_Date
